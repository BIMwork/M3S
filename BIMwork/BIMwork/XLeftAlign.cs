...
namespace BIMwork
{
    ...
    public class XLeftAlign : IExternalCommand
    {
        private const string X_LEFT_ALIGN = "X_LEFT_ALIGN";
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            try
            {
                ...
                using (Transaction trans = new Transaction(doc, X_LEFT_ALIGN))
                {
                    trans.Start();
                    // get minX
        ...
        private XYZ caclulateTargetPoint(ref Document doc, ref View view, ref List<ElementId> ids)
        {
            ...
            for (int i = 0; i < ids.Count; i++)
            {
                ...
                {
                    min = box.Min.X;
                    targetPoint = new XYZ(box.Min.X, box.Min.Y, box.Min.Z);
                }
            }
            return targetPoint;
        }
